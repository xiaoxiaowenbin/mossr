...
%}
%% MO model for BS
%%@(x,h,d,m,t) 表示这是一个匿名函数，是一个方便的方式来封装对 BS_model 函数的调用
CostFunction = @(x,h,d,m,t) BS_model(x,h,d,m,t);
nVar = 13;% number of bands
...
%重复放置Np个空粒子 repmat用于复制矩阵或者向量 矩阵为Np*1
pop = repmat(empty_particle, nPop, 1);         % population 

% 1st-generation population
for i = 1:nPop 
    %randperm(VarMax, nVar)生成一个从1到VarMax的随机排列，并返回前nVar个数。
    %sort函数用于对这些数进行排序，确保每个个体的位置（Position）是一个有序的变量集合。
...
% 将检测图和真值图转换为列向量
det_map_vec = detectmap(:);  % 转为 (W*H)*1 的列向量
GT_vec = img_gt(:);          % 转为 (W*H)*1 的列向量

% 调用cal_AUC函数
% mode_tau=1: 使用样本值作为阈值
% mode_eq=1: 使用方程(7) (>=tau为检测)
[AUC, AUCnor] = cal_AUC(det_map_vec, GT_vec, 1, 1);

% 输出结果
disp(['optimal band subset : (' num2str(fSolution),')']);
disp(['AUC (PF-PD): ' num2str(AUC.PFPD)]);
disp(['Normalized AUC (PF-PD): ' num2str(AUCnor.PFPD)]);
disp(['AUC (tau-PD): ' num2str(AUC.tauPD)]);
disp(['AUC (tau-PF): ' num2str(AUC.tauPF)]);