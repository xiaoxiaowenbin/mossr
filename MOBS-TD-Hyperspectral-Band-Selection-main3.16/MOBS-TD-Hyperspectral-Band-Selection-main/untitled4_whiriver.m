...
%% 3. 数据加载与预处理 (WHU-Hi-River 适配版)
disp('正在加载 WHU-Hi-River 数据...');

% --- 3.1 加载图像数据 ---
if exist(data_filename, 'file')
    [img_src, info] = enviread(data_filename);
    disp(['图像加载成功，尺寸: ' num2str(size(img_src))]);
else
    error(['找不到数据文件: ' data_filename]);
end
...
if exist(gt_filename, 'file')
    ...
    if sum(img_gt(:)) == 0
        error('真值图中无目标像素，请检查 target_mask。');
    end
else
    error(['找不到真值文件: ' gt_filename]);
end

% --- 3.3 尺寸校验 ---
[W, H, L] = size(img_src);
[rows, cols, num_bands] = size(img_src);  % 正确获取波段数
[Wg, Hg] = size(img_gt);

if W ~= Wg || H ~= Hg
    error('图像尺寸 (%dx%d) 与 真值尺寸 (%dx%d) 不匹配，请检查数据源。', W, H, Wg, Hg);
end

% --- 3.4 数据归一化 ---
N_pixels = W * H;
img_reshaped = reshape(img_src, N_pixels, L);

% 归一化 (Min-Max Normalization)
img_reshaped = double(img_reshaped);
min_val = min(img_reshaped(:));
max_val = max(img_reshaped(:));
img_reshaped = (img_reshaped - min_val) ./ (max_val - min_val + eps);

% --- 3.5 提取目标光谱 ---
% 使用提供的 get_target 函数提取先验光谱
d = get_target(img_reshaped, img_gt);

% --- 3.6 预计算相关矩阵 ---
disp('预计算相关矩阵 Y''Y (用于加速 fitness)...');
